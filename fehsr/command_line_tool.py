...
def main():
    ...
    if re.fullmatch(r'.*[^1]1\Z', args.unit_name):
        name_pattern = re.compile(r'(男|女|オリジナル|紋章|echoes|聖戦|トラキア|封印|烈火|蒼炎|暁|覚醒)?{}'.format(args.unit_name[:-1]))
    else:
        name_pattern = re.compile(r'(男|女|オリジナル|紋章|echoes|聖戦|トラキア|封印|烈火|蒼炎|暁|覚醒)?{}[0-9]*'.format(args.unit_name))
    searched_unit_list = [
